...
def operating_wrapper(master_filename: str, files: list[str], mapping: dict) -> str:
    manifest = []
...
> Compiler: `consolidate_project.py` v{COMPILER_VERSION} · Source documents: {len(files)} · Secrets: excluded
...
def build_master_files(mapping: dict, target_dir: Path) -> None:
    ...
    for master_filename, file_list in mapping.items():
        ...
        chunks = [operating_wrapper(master_filename, file_list, mapping)]
        for position, rel_file in enumerate(file_list, 1):
            payload, digest, _ = source_record(rel_file)
...
def main() -> None:
    targets = ((FIVE_FILE_MAP, BASE_DIR / "CONSOLIDATED_5_FILE_SYSTEM"), (NINE_FILE_MAP, BASE_DIR / "CONSOLIDATED_9_FILE_SYSTEM"))
    for mapping, target in targets:
        build_master_files(mapping, target)
